...
class RoomBooking(models.Model):
    ...
    need_service = fields.Boolean(default=True, string="Need Service",
    ...
    need_food = fields.Boolean(default=True, string="Need Food",
    ...
    amount_untaxed_room = fields.Monetary(string="Room Untaxed",
                                          help="Untaxed Amount for Room", store=True,
                                          compute='_compute_amount_untaxed',
                                          tracking=5)
    amount_untaxed_food = fields.Monetary(string="Food Untaxed", store=True,
    ...
    def _compute_amount_untaxed(self, flag=False):
        ...
        for rec in self:
            ...
            rec.amount_tax = 0.0
            rec.amount_taxed_food = 0.0
            rec.amount_taxed_room = 0.0
            rec.amount_taxed_fleet = 0.0
            rec.amount_taxed_event = 0.0
            rec.amount_taxed_service = 0.0
    ...
    def create_list(self, line_ids):
        ...
        account_move_line = self.env['account.move.line'].search_read(
            domain=[('ref', '=', self.name),
                    ('display_type', '!=', 'payment_term')],
            fields=['name', 'quantity', 'list_price', 'product_type'], )
    ...
    def action_invoice(self):
        ...
        food_lines = self.food_order_line_ids
        services_lines= self.service_line_ids
        room_line_ids = self.room_line_ids
        if booking_list:
            ...
            if food_lines:
                for food in food_lines:
                    ...
                    account_move.invoice_line_ids.create([{
                        'name': str(food['description']),
                        'quantity': food['uom_qty'],
                        'price_unit': food['list_price'],
                        'move_id': account_move.id,
                        'price_subtotal': food['uom_qty'] * food['list_price'],
                    }])
                    #Sale Order line creation
                    sale_order.order_line.create([{
                        'product_id': food['food_id'].id,
                        'product_template_id': food['food_id'].id,
                        'name': str(food['description']),
                        'product_uom_qty': food['uom_qty'],
                        'price_unit': food['list_price'],
                        'order_id': sale_order.id, #food['food_id'].location_id.id,
                        'order_partner_id':sale_order.partner_id,
                        'salesman_id': sale_order.user_id,
                        'state': 'order_id.state',
                        'price_subtotal': food['uom_qty'] * food['list_price'],
                    }])
                sale_order.action_confirm() #Sales Order confirmation
                stock_picking_ids = self.env['stock.picking'].search([('origin', '=', sale_order.name)]) #getting the picking realted to my sales

                for  stock_picking_id in stock_picking_ids:
                    for line in stock_picking_id.move_ids_without_package:
                        line.quantity_done = line.product_uom_qty
                    stock_picking_id.button_validate() #Validated my pi
            if services_lines:
                for service in services_lines:
                    #booking_list.append(self.create_list(food))
                    account_move.invoice_line_ids.create([{
                        'name': str(service['service_id'].name),

                        'quantity': service['uom_qty'],
                        'price_unit': service['list_price'],
                        'move_id': account_move.id,
                        'price_subtotal': service['uom_qty'] * service['list_price'],
                         }])
            #if room_line_ids:
            for room in room_line_ids:
                #booking_list.append(self.create_list(food))
                account_move.invoice_line_ids.create([{
                    'name': str(room['nuite_siete']),

                    'quantity': room['uom_qty'],
                    'price_unit': room['list_price'],
                    'move_id': account_move.id,
                    'price_subtotal': room['uom_qty'] * room['list_price'],
                     }])
